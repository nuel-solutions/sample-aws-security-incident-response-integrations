...
class ServiceNowApiService:
    ...
    def __get_password(self, password_param_name) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"Error retrieving ServiceNow password from SSM: {str(e)}")
            return None
    ...
    def __get_json_keys_list(self, json_string):
        ...
        try:
            ...
            logger.error(
                f"Error getting json keys list from the request: {str(e)}")
            return None

    def __add_outbound_rest_message_request_function_parameters(self, headers, base_url, request_content, outbound_rest_message_request_function_sys_id):
        ...
        try:
            ...
            request_content_parameters = self.__get_json_keys_list(
                request_content)
            ...
            for parameter in request_content_parameters:
                ...
                requests.post(f"{base_url}/api/now/table/sys_rest_message_fn_parameters",
                              json=rest_message_post_function_parameters_payload, headers=headers, timeout=30)
                logger.info(
                    f"Added parameter {parameter} to Http request function for Outbound REST Message")
        except Exception as e:
            logger.error(
                f"Error adding parameters to Http request function: {str(e)}")

    def __create_outbound_rest_message_request_function(self, headers, base_url, request_type, request_content, outbound_rest_message_name):
        ...
        try:
            ...
            rest_message_post_function_payload = {
                ...
                "function_name": f"{outbound_rest_message_name}-{request_type}-function",
                "http_method": f"{request_type}",
                "content": request_content
            }

            rest_message_post_function_response = requests.post(
                f"{base_url}/api/now/table/sys_rest_message_fn", json=rest_message_post_function_payload, headers=headers, timeout=30)
            ...
            rest_message_post_function_sys_id = rest_message_post_function_response_json.get(
                "result").get("sys_id")
    ...
    def _create_outbound_rest_message(self, webhook_url, resource_prefix):
        ...
        try:
            ...
            outbound_rest_message_response = requests.post(
                f"{base_url}/api/now/table/sys_rest_message", json=rest_message_payload, headers=headers, timeout=30)
            ...
            outbound_rest_message_request_function_sys_id = self.__create_outbound_rest_message_request_function(
                headers=headers, base_url=base_url, request_type=request_type, request_content=request_content, outbound_rest_message_name=outbound_rest_message_name)
            ...
            self.__add_outbound_rest_message_request_function_parameters(
                headers=headers, base_url=base_url, request_content=request_content, outbound_rest_message_request_function_sys_id=outbound_rest_message_request_function_sys_id)

            return outbound_rest_message_name
        except Exception as e:
            logger.error(
    ...
    def _create_incident_business_rule(self, outbound_rest_message_name, resource_prefix):
        ...
        (function executeRule(current, previous) {{
            try {{
                ...
                var request = new sn_ws.RESTMessageV2({outbound_rest_message_name});
        ...
        """
            ...
            response = requests.post(
                f"{base_url}/api/now/table/sys_script", json=rule_payload, headers=headers, timeout=30)
...
def handler(event, context):
    ...
    password_param_name = parameter_service.get_parameter(
    ...
    service_now_api_outbound_rest_message_name = service_now_api_service._create_outbound_rest_message(
        webhook_url, service_now_resource_prefix)

    if service_now_api_outbound_rest_message_name is None:
        logger.error(
        ...
        return {
            'Status': 'FAILED',
            'PhysicalResourceId': 'service-now-api-setup'
        }

    service_now_api_service._create_incident_business_rule(
        service_now_api_outbound_rest_message_name, service_now_resource_prefix)
